...
export class Clickup implements ReleaseStrategy {
  ...
  async fetchIssue(issue_id: IssueId): Promise<Issue> {
    ...
    if (pull_request.length === 0) {
      ...
      const pull_request_field = task.custom_fields?.find(
        (field) => field.name === "Pull Request"
      );
  ...
  async fetchIssues(list_id: IssueId, status?: string): Promise<Issue[]> {
    const { tasks } = await this.http.get<{ tasks: Task[] }>(
      `list/${list_id}/task${
        status ? `?statuses[]=${status}&order_by=updated&reverse=true` : ""
      }`
    );
    return tasks.map((task) => {
      let pull_request: string | undefined = github.getGitHubPR(
        task.description
      );
      if (pull_request.length === 0) {
        // if there is no pull request in description, try fetching it in the custom fields
      ...
      try {
        if (issue.pull_request) {
          ...
          if (is_merging_first_card) {
            ...
            logger.log(
              `Waiting ${MERGE_DELAY / 60000} minutes for build to finish...`,
              "loading"
            );
          ...
          if (!SHOULD_SKIP_CIRCLECI_CHECKS) {
            ...
            logger.log(
              "Skipping CircleCI release workflow checks based on settings..."
            );
          }
          merged_issues.push(issue);
        } else {
          throw new IssueError(
            IssueErrorType.NEEDS_PULL_REQUEST,
            issue,
            issue.assignees
          );
        }
  ...
  async createVersion(tag: string): Promise<Issue> {
    ...
    if (!has_release_tag_task) {
      logger.log(
        `Creating version ${PLATFORM} production_${tag}...`,
        "loading"
      );
  ...
  async addTaskRelationship(task_id: string, task_to_link_id: string) {
    const task = await this.http.post<Task>(
      `task/${task_id}/link/${task_to_link_id}`,
      {}
    );
  ...
  async getTemplates(space_id: string): Promise<Template[]> {
    const templates = await this.http.get<Template[]>(
      `team/${space_id}/taskTemplate?page=0`
    );
  ...
  async createRegressionTestingIssue(version: Issue): Promise<Issue> {
    ...
    const has_regression_testing_card = tasks.some(
    ...
    if (!has_regression_testing_card) {
      ...
      logger.log(
        `Creating regression testing card with title ${title}...`,
        "loading"
      );
      const task = await this.http.post<Task>(
        `list/${LIST_ID}/taskTemplate/${REGRESSION_TESTING_TEMPLATE_ID}`,
        {
          name: title,
        }
      );
      ...
      logger.log(
        `Regression testing card has already been created.`,
        "success"
      );
      regression_testing_card = tasks.find(
        (task) => task.title === title
      ) as Issue;
    }
