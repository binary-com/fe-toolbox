...
class CircleCI {
    ...
    async getPipelines(project_slug: string, branch: string): Promise<Pipeline[]> {
        const { items } = await this.http.get<PipelineResponse>(`project/${project_slug}/pipeline?branch=${branch}`);
    ...
    async getPipelineWorkflows(pipeline_id: string): Promise<Workflow[]> {
        const { items } = await this.http.get<WorkflowResponse>(`pipeline/${pipeline_id}/workflow`);
    ...
    async checkPipelineStatus(workflow_count: number = 15): Promise<void> {
        const pipelines = await this.getPipelines(CIRCLECI_PROJECT_SLUG, CIRCLECI_BRANCH);
        if (pipelines.length) {
            for (let i = 0; i < workflow_count; i++) {
                const pipeline = pipelines[i];
                const workflows = await this.getPipelineWorkflows(pipeline.id);
                const release_workflow = workflows.find(workflow => workflow.name === CIRCLECI_WORKFLOW_NAME);
                if (release_workflow) {
                    if (release_workflow.status === 'failed' && this.running_checks.has(pipeline.id)) {
                        throw new IssueError(IssueErrorType.FAILED_WORKFLOW, undefined, undefined);
                    }
