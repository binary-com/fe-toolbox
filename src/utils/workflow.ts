...
export class ReleaseWorkflow {
  ...
  logSummary(
    merged_issues: Issue[],
    failed_issues: IssueError[],
    failed_notifications: IssueError[]
  ) {
    ...
    if (failed_issues.length !== 0) {
      ...
      const rows: {
        [title: string]: { Reason: string; "Assignee notified": boolean };
      } = {};
        ...
        if (issue) {
          rows[issue.title] = {
            ...
            "Assignee notified": !failed_notifications.some(
              (failed_notification) =>
                failed_notification.issue?.id === issue.id
            ),
          };
        }
      });
  ...
  async run(): Promise<void> {
    try {
      let issues: Issue[] = await this.strategy.fetchIssues(
        LIST_ID,
        "ready - release"
      );
      if (issues.length === 0) {
      ...
      logger.log(
        `Release automation will start merging these ${issues.length} cards.`
      );
      ...
      if (!SHOULD_SKIP_SLACK_INTEGRATION) {
        try {
          ...
          logger.log(
            "There was an error in updating channel team_private_frontend.",
            "error"
          );
        }
      }
      ...
      if (merged_issues.length) {
        ...
        const tag_reqs = merged_issues.map((issue) =>
          this.strategy.addVersionToTask(issue, version)
        );
      ...
      if (failed_issues.length) {
          ...
          if (assignees) {
            assignees.forEach((assignee) => {
              if (assignee.email) {
                ...
                logger.log(
                  `Unable to notify assignee of ${failed_issue.issue?.title}`,
                  "error"
                );
              }
            });
            ...
            try {
              ...
              if (user) {
                await slack.sendMessage(
                  ...
                  loadUserHasFailedIssuesMsg(
                    user.name || "",
                    failed_issues_by_assignee[email]
                  )
                );
              } else {
              ...
              logger.log(
                `Unable to find user to notify for issue: ${err}`,
                "error"
              );
            }
          }

          const status_reqs = failed_issues_by_assignee[email].map(
            async ({ issue }) => {
              if (issue) {
                await clickup
        ...
        logger.log(
          `All assignees have been successfully notified of their issues!`
        );
      }
      ...
      if (!SHOULD_SKIP_SLACK_INTEGRATION) {
        try {
          ...
          logger.log(
            "There was an error in notifying channel task_release_planning_fe.",
            "error"
          );
        }
      }
