...
import { Assignee, Issue, IssueId, IssueQueue, ReleaseStrategy } from 'models/strategy';
...
class Redmine implements ReleaseStrategy {
    ...
    getIssueId(issue_link: string): IssueId | null {
        const groups = /https:\/\/redmine\.deriv\.cloud\/issues\/([0-9]+)/.exec(issue_link);
        if (groups) {
    ...
    async fetchUser(user_id: number): Promise<Assignee> {
        ...
        const response = await axios.get(`${REDMINE_API_URL}/users/${user_id}.json` || '', {
            headers: {
    ...
    async createVersion(): Promise<Version> {
        ...
        const version_tag = `V${today.getFullYear()}${today.getMonth() + 1}${today.getDate()}_${version_iteration}`;
        ...
        while (!this.issues_queue.is_empty) {
            ...
            try {
                ...
                if (redmine_ids?.includes(head?.id) || !redmine_ids) {
                    ...
                    logger.info(`Waiting ${MERGE_DELAY / 60000} minute for build to finish...`);
                    ...
                    logger.info(`CircleCI workflow checks completed for ${CIRCLECI_BRANCH} branch!`);
                } else {
    ...
    async enQueueCards(redmine_links: string[]) {
        ...
        const reqs_enqueues = redmine_links.map(redmine_link => {
            try {
                ...
                throw new Error(`Unable to fetch ${redmine_link}: ${err instanceof Error ? err.message : err}`);
            }
        });
