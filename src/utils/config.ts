...
export const CIRCLECI_TOKEN =
  process.env.CIRCLECI_TOKEN ||
  core.getInput("CIRCLECI_TOKEN", { required: true });
export const CLICKUP_API_TOKEN =
  process.env.CLICKUP_API_TOKEN ||
  core.getInput("CLICKUP_API_TOKEN", { required: true });
...
export const SLACK_APP_TOKEN =
  ...
  core.getInput("SLACK_APP_TOKEN", { required: true });
export const SLACK_BOT_TOKEN =
  process.env.SLACK_BOT_TOKEN ||
  core.getInput("SLACK_BOT_TOKEN", { required: true });
export const SLACK_USER_TOKEN =
  process.env.SLACK_USER_TOKEN ||
  core.getInput("SLACK_USER_TOKEN", { required: true });
export const SLACK_SIGNING_SECRET = process.env.SLACK_SIGNING_SECRET || "";
export const GITHUB_REPO = process.env.GITHUB_REPO || github.context.repo.repo;
export const GITHUB_REPO_OWNER =
  process.env.GITHUB_REPO_OWNER || github.context.repo.owner;
...
export const REGRESSION_TESTING_TEMPLATE_ID = core.getInput(
...
export const PLATFORM =
  core.getInput("platform", { required: false }) || "Deriv.app";
...
export const SHOULD_SKIP_PENDING_CHECKS =
  core.getInput("skip_pending_checks", { required: false }) === "true" ||
  config?.skip_pending_checks ||
  false;
export const SHOULD_SKIP_CIRCLECI_CHECKS =
  ...
  false;
export const SHOULD_SKIP_UPDATING_BRANCH =
  config?.skip_updating_branch || false;
export const SHOULD_SKIP_SLACK_INTEGRATION =
  config?.skip_slack_integration || false;
...
export const FIRST_MERGE_DELAY = config?.first_merge_delay || 20 * 60 * 1000;
export const PULL_REQUEST_CHECKS_TIMEOUT =
  config?.pull_request?.checks_timeout || 1 * 60 * 1000; // 1 minute
export const PULL_REQUEST_REFETCH_TIMEOUT =
  config?.pull_request?.refetch_timeout || 5 * 1000; // 5 seconds
export const PULL_REQUEST_REFETCH_LIMIT =
  config?.pull_request?.refetch_limit || 10; // the max amount of refetches to check for a pull request's status checks
export const PULL_REQUEST_CHECKS_LIMIT =
  config?.pull_request?.checks_limit || 120;
